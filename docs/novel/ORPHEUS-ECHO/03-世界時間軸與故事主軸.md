...
>
> 角色的年齡、性格、狀態與外觀設定請看 [00-角色設定.md](00-角色設定.md)。
...
這裡只說明角色和事件的關係；人物詳細設定留在 `00-角色設定.md`。
...
玩家輸入的名字是他們當下選擇使用的稱謂，受試編號則是研究系統早已存在的識別方式。前期兩者不會被明白地連在一起，直到後期文件出現「受試編號自行選擇稱謂」的交叉紀錄，玩家才知道那個編號就是自己。
...
5. `00-角色設定.md` 只定義角色本身；角色和事件、角色和文件的關係，統一回到本文件維護。
