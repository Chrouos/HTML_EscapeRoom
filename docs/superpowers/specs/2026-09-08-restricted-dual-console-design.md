...
AI 可靠且冷靜地說明工作站，強調兩名玩家必須合作。公開任務比私人任務多，讓玩家先建立共同目標。

開場至少完成兩輪公開 AI 說明與一個共同操作後，才允許第一個私人事件。每個主線階段至少提供 A、B 各一個可執行的角色相關操作；同一階段每個角色最多新增一個私人任務，避免私人內容壓過共同解謎。

### 第二幕：建立關係

AI 從固定句庫挑選能回應近期操作的閒聊，例如記得玩家剛開過的檔案、稱讚一次校驗或詢問對目前實驗的看法。對話仍是預先編寫內容，不使用即時生成。

### 第三幕：合理懷疑

文件開始揭露單人撤離協定、身份欄位異常、未登記存取和實驗分組差異。AI 不總是直接指控，而是針對玩家已看到的異常提出問題，讓猜疑看似由證據自然產生。

### 第四幕：私人任務滲入閒聊

共有任務與私人任務交錯。私人任務可能被包裝成順手整理資料、避免打擾同伴或修復污染紀錄的普通請求。部分指令為真、部分過時、部分刻意偽造。
...
```js
{
  ...
  mainlineFallbackActionIds: [],
  debriefFactIds: []
}
```

每個可執行操作另有 action manifest：
...
- 主線 action、公開 unlock 與私人 action 形成有向依賴圖；validator 必須能從初始節點計算 reachability 和循環。
...
- edge 由 action 的 `unlockWhen` 指向 action，再由 action 指向 `effects` 中的 fact、entry 和 completed node。不存在於 manifest 的隱含 side effect 不得參與主線或結局。
- 誤導內容的 `verificationEntries` 至少一項必須可達，而且 `sourceGroup` 必須不同於誤導來源。
- 私人任務的 `requiresPrivateFacts` 不得被任何主線 action 引用。

句庫依宣告順序保存。選擇 index 使用 SHA-256 對 UTF-8 字串 `${roomCode}:${playerId}:${contentId}` 雜湊，取前 8 個十六進位字元轉成 unsigned integer，再對候選句數取餘數。不同房間看似有變化，同一房間重連後不會換句話，跨 Node 版本也能使用固定 fixture 測試。
...
| 任務 | 觸發 | 允許的 action ID | 主線 fallback action ID |
|---|---|---|---|
| A-1 | A 第一次開啟主線要求的 Files 目錄 | `archive_index`、`decline_index_repair`、`skip_a1` | `continue_file_index` |
| B-1 | B 第一次開啟主線要求的實驗名冊 | `flag_identity`、`share_roster`、`decline_identity_check`、`skip_b1` | `continue_roster_review` |
| A-2 | A 第一次提交主線校驗，無論正誤 | `delete_local_mirror`、`share_mirror_first`、`decline_mirror_cleanup`、`skip_a2` | `restore_mirror_from_backup` |
...
| A-1 | 私人任務 | 修復索引 | 無事件痕跡 | 中性操作被包裝成緊急任務 | A 首次開 Files | 原始索引時間、B 校驗碼 | 可忽略 | `aArchivedIndex` |
| B-1 | 文件＋私人對話 | 無事件痕跡 | A 身份欄位異常 | 過期資料，部分真實 | B 開名冊 | 人事異動紀錄、A 附件 | 可轉送或忽略 | `bFlaggedIdentity` |
| D-1 | 同名異版文件 | 事故發生於 02:17 前 | 事故發生於 02:17 後 | 兩份都經剪輯 | 共同謎題 2 | 原始錄音 timestamp | Logs 可取得摘要 | `comparedIncidentTimes` |
| A-2 | 私人任務 | 刪除污染鏡像 | 無事件痕跡 | 鏡像未污染，只是 checksum 不同 | A 第一次提交校驗，無論答案正誤 | B 版本、系統備份 | Terminal 手動還原 | `aDeletedMirror`、`aSharedMirrorFirst` |
...
- validator 移除全部 private action edges 後，仍必須能從初始狀態走到 `finale_ready` 與雙方 `commit_finale`；再對每個私人任務逐一移除所有 outcome edges，結果也必須成立。
- 每個私人任務都列出至少一個 `mainlineFallbackActionId`，且該 action 的 unlock graph 不得依賴 private fact。
- audience 必填且只能使用允許的 schema。
- 內容 ID 唯一；觸發條件不形成無法到達或循環依賴。
- 六個私人任務各只能建立一次。
- action manifest 的每個 effect target 都必須存在；從固定初始 facts traversal 時，四個成功節點都必須可達。移除 `kind: 'private'` 的 edges 後，兩個 `neutral_finale` commit edge 仍必須可達並能把 pending mission 原子記為 skipped。
...
4. 六個私人任務已接入共同劇情；刪除全部私人 action edges 後仍能完成主線與提交撤離程序。
