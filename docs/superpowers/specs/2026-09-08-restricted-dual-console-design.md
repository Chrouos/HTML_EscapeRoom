...
- `AI_BROADCAST`：AI 對兩名玩家發布的相同公告、共同任務與系統說明；雙方使用相同內容 ID 與文字。
- `AI_DIRECT`：只屬於目前玩家的閒聊、觀察、挑撥與私人任務。另一方知道 AI 存在，但不知道私人頻道何時發話或說了什麼。
- 連線狀態、訊號干擾與傳送回饋。
...
公開 AI 並不消失：後續共同任務、倒數警告與系統公告仍使用 `AI_BROADCAST` 同步送給雙方。只有垃圾話、關係建立、角色觀察、私人任務與挑撥內容使用 `AI_DIRECT`。
