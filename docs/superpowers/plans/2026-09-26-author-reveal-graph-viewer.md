...
**Architecture:** A pure server-side adapter derives a canonical technical graph from current content. A presentation layer collapses implementation-only facts/gates into story-language nodes and edges, and a diagnostics layer reports structural problems. Express exposes the result only outside production; EJS + vanilla JavaScript + SVG render deterministic reveal-stage columns and A/B/shared/ECHO swimlanes.
...
- Predicate structure must preserve the existing `all` / `any` / `not` semantics from `game/content/contentSchema.js`; `not` must remain a NOT gate rather than being distributed across children.
- Exact ending resolution remains labeled `runtime-calculated`.
- The existing `npm run check` suite must remain green.

## Review Focus

- Nested predicates and one-time guards: preserve logical structure while hiding noisy technical nodes in simple mode; Task 1 pins `ALL`, `ANY`, `NOT`, and `reactionFactMissing`.
- Missing presentation metadata: fall back to a deterministic readable label rather than blank cards; Task 2 pins fallback behavior.
- A/B privacy boundaries: true leaks warn, explicit sharing actions do not; Task 3 pins both cases.
- Production isolation: both author endpoints remain 404 even though the router is mounted; Task 4 pins both routes.
- Sparse or malformed graph data: keep the page usable and show a human-readable partial-data state; Tasks 4 and 5 pin server/client fallbacks.
...
game/authoring/storyGraphPresentation.js
  Human labels, reveal stages, swimlanes, hidden-node collapse.
...
**Interfaces:**
- Consumes: `content` + `isPredicateShapeValid` from `game/content/contentSchema.js`, and `endings` from `game/content/endings.js`.
...
test('builds namespaced nodes from canonical content', () => {
...
test('maps all operation effect families', () => {
  const graph = buildTechnicalStoryGraph({ contentBundle: content, endingCatalog: endings });
...
Expected: FAIL with module-not-found for `game/authoring/storyGraph.js`.

- [ ] **Step 3: Implement the minimal node/effect graph builder**
...
function buildTechnicalStoryGraph({ contentBundle, endingCatalog }) {
  ...
  for (const entry of contentBundle.terminalEntries || []) {
    addNode(nodeMap, {
      id: `file:${entry.id}`, refId: entry.id, technicalType: 'FILE', audience: entry.audience,
      source: { module: 'game/content/terminalEntries.js', contentFile: entry.contentFile || null }, raw: entry
    });
  }
  ...
  for (const operation of contentBundle.operations || []) {
    ...
    for (const effectFamily of Object.keys(EFFECT_PREFIX)) {
      for (const value of operation.effects?.[effectFamily] || []) {
        const id = `${EFFECT_PREFIX[effectFamily]}:${value}`;
        const type = {
          publicFacts: 'FACT', roleFacts: 'ROLE_FACT', unlockEntryIds: 'FILE',
          completeNodeIds: 'COMPLETION', appendContentIds: 'CONTENT'
        }[effectFamily];
        addNode(nodeMap, { id, refId: value, technicalType: type });
        edges.push({ from: actionId, to: id, kind: EFFECT_KIND[effectFamily], polarity: 'positive', detail: null });
      }
    }
  }
  ...
  for (const line of contentBundle.dialogue || []) {
    addNode(nodeMap, {
      id: `echo:${line.id}`, refId: line.id, technicalType: 'ECHO', audience: line.audience,
      source: { module: 'game/content/dialogue.js' }, raw: line
    });
  }
...
- [ ] **Step 4: Add failing predicate tests that pin logical structure**

```js
test('simple fact prerequisite is a direct technical edge', () => {
  const result = normalizePredicate({ publicFact: 'main5Completed' }, 'file:archive.history_timeline');
  assert.deepEqual(result.edges, [{
    from: 'fact:main5Completed', to: 'file:archive.history_timeline',
    kind: 'REQUIRES', polarity: 'positive', detail: null
  }]);
});

test('nested ALL and ANY become explicit gates', () => {
  const result = normalizePredicate({
    all: [
      { publicFact: 'main1Completed' },
      { any: [{ actionAttempted: 'share_roster' }, { actionAttempted: 'warn_partner_first' }] }
    ]
  }, 'echo:example');
...
test('NOT remains a NOT gate and is not distributed across child logic', () => {
  const result = normalizePredicate({
    not: {
      all: [{ publicFact: 'mainCompleted' }, { actionAttempted: 'commit_finale' }]
    }
  }, 'echo:example');
...
test('reactionFactMissing is represented as an absent-condition leaf', () => {
  const result = normalizePredicate({ reactionFactMissing: 'echo.behavior.protocol_recheck' }, 'echo:example');
  assert.deepEqual(result.edges[0], {
    from: 'reaction:echo.behavior.protocol_recheck', to: 'echo:example',
    kind: 'REQUIRES', polarity: 'negative', detail: null
  });
});

test('entryOpenedTimes and idle thresholds keep detail', () => {
  assert.deepEqual(
...
- [ ] **Step 5: Run the predicate tests and verify they fail**

```bash
node --test test/unit/authorStoryGraph.test.js
```

Expected: FAIL on missing predicate normalization.

- [ ] **Step 6: Implement predicate normalization with explicit NOT/ALL/ANY gates**
...
function normalizePredicate(predicate, ownerId, path = 'unlock') {
  ...
  if (predicate.not !== undefined) {
    ...
    return {
      ...
      edges: [...child.edges, { from: gate.id, to: ownerId, kind: 'REQUIRES', polarity: 'positive', detail: null }],
      errors: child.errors
    };
  }
  ...
  return { gates: [], edges: [{ from, to: ownerId, kind: 'REQUIRES', polarity, detail }], errors: [] };
}
```

Call `normalizePredicate()` for every `terminalEntry.unlockWhen`, `dialogue.unlockWhen`, and `operation.unlockWhen`. Add returned gate nodes as `technicalType: 'GATE'`, and add referenced leaf nodes when missing.

- [ ] **Step 7: Run Task 1 and existing unit tests**
...
### Task 2: Convert the technical graph into a human Story Map
...
- Visible types: `DISCOVERY`, `ECHO`, `ACTION`, `TRUTH`, `ENDING`.
- Visible lanes: `shared`, `A`, `B`, `ECHO`.

- [ ] **Step 1: Add failing tests for hidden technical nodes, human labels, lanes, and fallback labels**
...
test('simple Story Map hides implementation-only nodes', () => {
...
test('missing metadata has deterministic readable fallback', () => {
  const story = buildStoryMapModel({
    nodes: [{ id: 'action:verify_incident_timestamp', refId: 'verify_incident_timestamp', technicalType: 'ACTION' }],
    edges: [], stats: {}
  }, { nodes: {} });
...
- [ ] **Step 2: Run and verify presentation modules are absent**

```bash
node --test test/unit/authorStoryGraph.test.js
```

Expected: FAIL with module-not-found for presentation modules.

- [ ] **Step 3: Create the human stage catalog and the v1 core metadata**

`game/authoring/storyGraphMetadata.js` must contain at least these authored nodes:
...
- [ ] **Step 4: Implement presentation helpers and milestone inference**
...
const MILESTONE_STAGE = Object.freeze({
  roomCreated: 'R0', main1Completed: 'R1', main2Completed: 'R2', main3Completed: 'R3',
  main4Completed: 'R4', main5Completed: 'R5', mainCompleted: 'R6', finale_ready: 'R6'
});

function humanizeId(refId = '') {
  return refId.replace(/^[^.]+\./, '').replace(/[._-]+/g, ' ').trim().replace(/^\w/, char => char.toUpperCase());
}
...
Infer fallback stage from positive prerequisite/effect paths anchored by `MILESTONE_STAGE`; authored metadata overrides mechanical stage when story meaning intentionally differs.

- [ ] **Step 5: Add failing tests for hidden-path collapse and runtime ending labeling**

```js
test('hidden technical paths collapse into visible human edges', () => {
  const story = buildStoryMapModel(buildTechnicalStoryGraph({ contentBundle: content, endingCatalog: endings }), storyGraphMetadata);
...
test('ending resolution is marked runtime-calculated without copied conditions', () => {
  const story = buildStoryMapModel(buildTechnicalStoryGraph({ contentBundle: content, endingCatalog: endings }), storyGraphMetadata);
...
- [ ] **Step 6: Implement hidden-node collapse with technical-path preservation**

Walk backwards through hidden nodes (`FACT`, `ROLE_FACT`, `GATE`, `COMPLETION`, `CONTENT`) with a visited set. Visible edges use story labels (`解鎖`, `因為看過`, `因為選擇`, `ECHO 注意到`, `可能導向`) and retain:
...
If no visible upstream node exists, preserve raw prerequisite IDs under `node.technical.prerequisites` rather than showing fact cards.

- [ ] **Step 7: Run Task 2 + unit suite**
...
### Task 3: Add all structural diagnostics
...
- [ ] **Step 1: Write failing tests covering every diagnostic family**
...
test('BROKEN_REFERENCE checks operation, predicate, and verification entry refs', () => {
  ...
  assert.ok(refs.includes('missing.operation.entry'));
  assert.ok(refs.includes('missing.predicate.entry'));
  assert.ok(refs.includes('missing.verification.entry'));
});

test('UNREACHABLE reports visible content with a never-produced prerequisite', () => {
  const broken = structuredClone(content);
  ...
  assert.ok(diagnose(broken).some(i => i.code === 'UNREACHABLE' && i.nodeIds.includes('file:doc.unreachable')));
});

test('ORPHAN reports isolated content but excludes intentional initial content', () => {
  const broken = structuredClone(content);
...
test('AUDIENCE_LEAK distinguishes leak from canonical sharing action', () => {
  const broken = structuredClone(content);
...
test('MISSING_VERIFICATION applies to observation/manipulation claims', () => {
  const broken = structuredClone(content);
  broken.dialogue.find(line => line.id === 'orpheus.observation.a').verificationEntries = [];
  assert.ok(diagnose(broken).some(i => i.code === 'MISSING_VERIFICATION' && i.nodeIds.includes('echo:orpheus.observation.a')));
});

test('ENDING_DEBRIEF_GAP reports missing debrief fact', () => {
  const catalog = structuredClone(endings);
  catalog.cooperative_escape.debriefFactIds = ['missingDebriefFact'];
  assert.ok(diagnose(content, catalog).some(i => i.code === 'ENDING_DEBRIEF_GAP'));
});

test('SUSPICIOUS_EARLY_REVEAL stays advisory', () => {
  const broken = structuredClone(content);
  broken.terminalEntries.find(entry => entry.id === 'files.mainline').verificationEntries = [{ entryId: 'archive.history_timeline', sourceGroup: 'history_timeline' }];
  assert.ok(diagnose(broken).some(i => i.code === 'SUSPICIOUS_EARLY_REVEAL' && i.severity === 'hint'));
});
```

- [ ] **Step 2: Run and verify diagnostics module is absent**
...
- [ ] **Step 3: Implement diagnostic factory and exact human-first titles**
...
function issue(code, severity, message, nodeIds = [], technical = {}) {
  return {
    ...
    code, severity, title: HUMAN_TITLES[code], message,
    nodeIds, edgeIds: [], technical
  };
}
```
...
```text
BROKEN_REFERENCE
  Validate unlockEntryIds, entryOpened/entryOpenedTimes refs, verificationEntries.
UNREACHABLE
  Traverse positive graph dependencies from initial/default content and roomCreated; negative guards never establish reachability.
ORPHAN
  Visible node has no meaningful incoming/outgoing story edge after collapse; exclude intentional initial/default content and endings.
AUDIENCE_LEAK
  Shared node directly depends on A/B-private input without an explicit share action.
  Allowed share actions: share_roster, share_mirror_first, warn_partner_first, request_pair_validation, disclose_report.
MISSING_VERIFICATION
  Dialogue intent is observation/manipulation, text is non-empty, verificationEntries is empty.
ENDING_DEBRIEF_GAP
  ending.debriefFactIds entry is missing from contentBundle.debrief by factId.
SUSPICIOUS_EARLY_REVEAL
  Verification target is at least two reveal stages later; severity is hint.
```

- [ ] **Step 4: Attach diagnostics to the Story Map**
...
Export it from `storyGraphPresentation.js`.

- [ ] **Step 5: Run diagnostics + full unit suite**
...
### Task 4: Expose development/test-only author routes
...
**Interfaces:**
- `GET /author/reveal-graph` -> page shell outside production.
- `GET /author/api/reveal-graph` -> model JSON outside production.
- Build error -> status 200 partial diagnostic model in development/test.
- Production -> router exits and existing app returns 404.

- [ ] **Step 1: Write failing integration tests for access, production isolation, and build-error fallback**
...
let runningServer;
test.before(async () => { runningServer = await testServer(app); });
test.after(async () => { if (runningServer) await runningServer.close(); });
...
test('production hides both endpoints', async () => {
  ...
  try {
    assert.equal((await fetch(`${runningServer.baseUrl}/author/reveal-graph`)).status, 404);
    assert.equal((await fetch(`${runningServer.baseUrl}/author/api/reveal-graph`)).status, 404);
  } finally {
    app.set('env', original);
  }
});

test('build failure returns partial diagnostic model', async () => {
  const probe = express();
  probe.set('env', 'test');
...
- [ ] **Step 2: Run and verify routes module is absent**
...
- [ ] **Step 3: Implement model composition, fallback model, and production guard**
...
function failedModel(error) {
  return {
    ...
    diagnostics: [{
      ...
      title: '部分故事資料無法解析', message: '部分故事資料無法解析；請查看技術細節確認內容格式。',
...
function createAuthorRoutes({ buildModel = defaultBuildModel } = {}) {
  ...
  router.get('/api/reveal-graph', authorOnly, (request, response) => {
    try { response.json(buildModel()); }
    catch (error) { response.json(failedModel(error)); }
  });
  return router;
}
...
- [ ] **Step 4: Mount router and create page shell**
...
Mount before `pageRoutes`/404 handling.
...
- [ ] **Step 5: Run Task 4 + server-side suites**
...
### Task 5: Render deterministic SVG swimlanes
...
- [ ] **Step 1: Write failing E2E for human-readable map and empty data**

```js
const { test, expect } = require('@playwright/test');

test('Story Map loads human stages and core story nodes', async ({ page }) => {
...
test('empty graph keeps page usable', async ({ page }) => {
...
- [ ] **Step 2: Run and verify interactive UI is absent**

```bash
npx playwright test test/e2e/authorRevealGraph.spec.js
```

Expected: FAIL on missing filter/node UI.

- [ ] **Step 3: Add accessible filter/canvas/inspector regions to EJS**
...
<section class="story-map-toolbar" aria-label="Story Map 篩選器">
  ...
  <label>故事階段<select data-stage-filter>
    <option value="all">全部</option><option value="R0">進入實驗</option><option value="R1">建立合作</option>
    <option value="R2">產生矛盾</option><option value="R3">ECHO 介入</option><option value="R4">身分揭露</option>
    <option value="R5">選擇框架</option><option value="R6">結果</option>
  </select></label>
  <label>類型<select data-story-type-filter>
    <option value="all">全部</option><option value="DISCOVERY">發現</option><option value="ECHO">ECHO</option>
    <option value="ACTION">行動</option><option value="TRUTH">真相</option><option value="ENDING">結局</option>
  </select></label>
...
<section class="story-map-workspace">
  ...
  <aside class="story-map-inspector" data-story-map-inspector aria-live="polite"><h2>選擇一段故事</h2></aside>
</section>
<footer data-story-map-diagnostics></footer>
```

- [ ] **Step 4: Implement loading, filtering, layout, and SVG rendering**
...
(() => {
  ...
  function filterModel(model, filters = {}) {
    ...
    const nodes = (model.nodes || []).filter(node => {
      ...
      if (filters.diagnosticsOnly && !diagnosticIds.has(node.id)) return false;
      return true;
    });
  ...
  function layoutNodes(model) {
    ...
    return (model.nodes || []).map(node => {
      const stage = Math.max(0, STAGE_ORDER.indexOf(node.stage));
      const lane = Math.max(0, LANE_ORDER.indexOf(node.lane));
      const key = `${node.stage}:${node.lane}`;
      const offset = counters.get(key) || 0;
      counters.set(key, offset + 1);
      return { ...node, x: 160 + stage * 290, y: 110 + lane * 190 + offset * 76 };
    });
  }
  ...
  function renderStoryMap(model, root) {
    ...
    if (!nodes.length) { root.innerHTML = '<p data-empty-story-map>這個篩選條件下沒有故事節點。</p>'; return; }
    const byId = new Map(nodes.map(node => [node.id, node]));
    const height = Math.max(820, ...nodes.map(node => node.y + 100));
    const edges = (model.edges || []).map(edge => [edge, byId.get(edge.from), byId.get(edge.to)])
      .filter(([, from, to]) => from && to)
      .map(([edge, from, to]) => `<path class="story-edge" d="M ${from.x + 220} ${from.y + 28} C ${from.x + 250} ${from.y + 28}, ${to.x - 30} ${to.y + 28}, ${to.x} ${to.y + 28}" />`).join('');
    const cards = nodes.map(node => `<g class="story-node" tabindex="0" role="button" data-node-id="${escapeHtml(node.id)}" data-story-node="${escapeHtml(node.refId)}" data-lane="${escapeHtml(node.lane)}" data-stage="${escapeHtml(node.stage)}" transform="translate(${node.x} ${node.y})"><rect width="220" height="58" rx="10"></rect><text x="12" y="24">${escapeHtml(node.label)}</text><text class="story-node-meta" x="12" y="43">${escapeHtml(node.stage)} · ${escapeHtml(node.lane)}</text></g>`).join('');
    root.innerHTML = `<svg class="story-map-svg" viewBox="0 0 2200 ${height}" aria-label="ORPHEUS 故事流程圖">${edges}${cards}</svg>`;
  }
...
Add `load()` that fetches `/author/api/reveal-graph`; on fetch/parse failure set `[data-story-map-status]` to `部分故事資料無法解析。` and keep the shell visible.

- [ ] **Step 5: Add readable CSS**
...
- [ ] **Step 6: Run focused E2E and commit**
...
Expected: E2E PASS before commit.

---

### Task 6: Add inspector, filters, diagnostics focus, and technical drill-down

**Files:**
- Modify: `public/js/authorRevealGraph.js`
- Modify: `public/css/authorRevealGraph.css`
- Modify: `test/e2e/authorRevealGraph.spec.js`
- Modify only if an asserted human label is missing: `game/authoring/storyGraphMetadata.js`

- [ ] **Step 1: Add failing E2E for viewpoint/stage/type filters**

```js
test('filters narrow the story without exposing technical fact nodes', async ({ page }) => {
  await page.goto('/author/reveal-graph');
...
- [ ] **Step 2: Add failing E2E for story-first inspector and keyboard selection**
...
- [ ] **Step 3: Implement state, filter bindings, node selection, and inspector**
...
function section(titleText, bodyText) {
  ...
  title.textContent = titleText; body.textContent = bodyText;
  wrapper.append(title, body); return wrapper;
}
...
function renderInspector(node, model, diagnostic = null) {
  ...
  const later = outgoing.map(edge => model.nodes.find(item => item.id === edge.to)).filter(Boolean).slice(0, 4);
  ...
  toggle.type = 'button'; toggle.textContent = '顯示技術細節'; details.hidden = true;
  details.textContent = JSON.stringify({ id: node.refId, source: node.source, stage: node.stage, lane: node.lane, technical: node.technical, incoming: incoming.map(e => e.technicalPath || e.kind), outgoing: outgoing.map(e => e.technicalPath || e.kind) }, null, 2);
  toggle.addEventListener('click', () => { details.hidden = !details.hidden; toggle.textContent = details.hidden ? '顯示技術細節' : '隱藏技術細節'; });
  inspector.append(toggle, details);
}
```

Bind click + Enter/Space to `[data-node-id]`, and bind all four filters to `filterModel()` + rerender.

- [ ] **Step 4: Add failing diagnostic-focus E2E**

```js
test('diagnostic selection explains and focuses its story node', async ({ page }) => {
...
- [ ] **Step 5: Render diagnostics and implement focus behavior**
...
Render diagnostics as buttons whose accessible name starts with the human `title`; show technical `code` only as secondary text. Diagnostics-only mode keeps nodes referenced by diagnostics and direct edges between retained nodes.

- [ ] **Step 6: Run focused E2E and full verification**
...
- [ ] **Step 7: Scan for accidental second ending source of truth or graph dependency**
...
Expected: no copied ending-resolution implementation and no new graph-library dependency.
...
[ ] All seven specified diagnostics exist with human-first messages.
[ ] Explicit sharing actions do not create false audience-leak warnings.
[ ] Exact ending resolution is runtime-calculated; ending precedence is not duplicated.
[ ] HTML and JSON author routes return 404 in production.
[ ] Build failure and empty graph data keep the author page usable.
